...
/**
 * NOTE(ljs):fresco保存的缓存文件是以cnt结尾的，拿到文件后只要另存为jpg或png文件即可
 *
 * @author notreami
 *
 */
public class ConfigConstants {
  ...
  private static final String FRESCO_DEFAULT_IMG_DIR = "fresco_default_img_dir";// 默认图所放路径的文件夹名
  private static final String FRESCO_SMALL_IMG_DIR = "fresco_small_img_dir";// 小图所放路径的文件夹名
  ...
  private static ImagePipelineConfig configureCaches(Context context) {
    // 内存配置
    final MemoryCacheParams bitmapCacheParams = new MemoryCacheParams(ConfigConstants.MAX_MEMORY_CACHE_SIZE, // 内存缓存中总图片的最大大小,以字节为单位。
    ...
    // 小图片的磁盘配置,/data/data/packagename/cache
    DiskCacheConfig diskSmallCacheConfig = DiskCacheConfig.newBuilder().setBaseDirectoryPath(context.getApplicationContext().getCacheDir())// 缓存图片基路径
        .setBaseDirectoryName(FRESCO_SMALL_IMG_DIR)// 文件夹名
    ...
    // 默认图片的磁盘配置,/storage/emulated/0-->Environment.getExternalStorageDirectory().getAbsoluteFile()
    DiskCacheConfig diskCacheConfig = DiskCacheConfig.newBuilder().setBaseDirectoryPath(Environment.getExternalStorageDirectory().getAbsoluteFile())// 缓存图片基路径
        .setBaseDirectoryName(FRESCO_DEFAULT_IMG_DIR)// 文件夹名
        // .setCacheErrorLogger(cacheErrorLogger)//日志记录器用于日志错误的缓存。
        // .setCacheEventListener(cacheEventListener)//缓存事件侦听器。
        // .setDiskTrimmableRegistry(diskTrimmableRegistry)//类将包含一个注册表的缓存减少磁盘空间的环境。
        .setMaxCacheSize(ConfigConstants.MAX_DISK_CACHE_SIZE)// 默认缓存的最大大小。
    ...
    // 缓存图片配置
    ImagePipelineConfig.Builder configBuilder = ImagePipelineConfig.newBuilder(context).setBitmapMemoryCacheParamsSupplier(mSupplierMemoryCacheParams)// 内存缓存配置（一级缓存，已解码的图片）
  ...
  public static RoundingParams getRoundingParams() {
    ...
    // roundingParams.setBorder(color, width);//fresco:roundingBorderWidth="2dp"边框
   ...
   * .setLowestPermittedRequestLevel(RequestLevel.FULL_FETCH)//请求经过缓存级别
   * BITMAP_MEMORY_CACHE，ENCODED_MEMORY_CACHE，DISK_CACHE，FULL_FETCH //
   * .setPostprocessor(postprocessor)//修改图片 //
   * .setProgressiveRenderingEnabled(true)//渐进加载，主要用于渐进式的JPEG图，影响图片显示速度（普通） .setResizeOptions(new
   * ResizeOptions(view.getLayoutParams().width, view.getLayoutParams().height))//调整大小 //
   * .setSource(Uri uri)//设置图片地址 .build(); return imageRequest; }
   */

  // DraweeController 控制 DraweeControllerBuilder
  /*
   * public static DraweeController getDraweeController(ImageRequest
   * imageRequest,InstrumentedDraweeView view){ DraweeController draweeController =
   * Fresco.newDraweeControllerBuilder() // .reset()//重置 .setAutoPlayAnimations(true)//自动播放图片动画 //
   * .setCallerContext(callerContext)//回调 .setControllerListener(view.getListener())//监听图片下载完毕等 //
   ...
   * .setImageRequest
   * (imageRequest)//设置单个图片请求～～～不可与setFirstAvailableImageRequests共用，配合setLowResImageRequest为高分辨率的图
   * // .setLowResImageRequest(ImageRequest.fromUri(lowResUri))//先下载显示低分辨率的图
   * .setOldController(view.getController())//DraweeController复用
   * .setTapToRetryEnabled(true)//点击重新加载图 .build(); return draweeController; }
   */

  /*
   * //默认加载图片和失败图片 public static Drawable sPlaceholderDrawable; public static Drawable
   * sErrorDrawable;
   *
   * @SuppressWarnings("deprecation") public static void init(final Resources resources) { if
   * (sPlaceholderDrawable == null) { sPlaceholderDrawable =
   * resources.getDrawable(R.color.placeholder); } if (sErrorDrawable == null) { sErrorDrawable =
   * resources.getDrawable(R.color.error); } }
   */
